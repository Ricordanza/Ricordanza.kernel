...
namespace Ricordanza.Core.Images
{
    ...
    public static class ImageUtility
    {
        ...
        public static Bitmap GetPalette(this Bitmap self)
        {
            if (self == null)
            ...
            ColorPalette pal = self.Palette;
            Bitmap palb = new Bitmap(r * 16 + m * 2 + 1, r * 16 + m * 2 + 1,
                                               PixelFormat.Format24bppRgb);

            using (Graphics g = Graphics.FromImage(palb))
            {
                g.Clear(Color.FloralWhite);

                using (SolidBrush br = new SolidBrush(Color.AliceBlue))
                {
                ...
                using (Pen p = new Pen(Color.Silver, 1f))
                {
                    for (int y = 0; y < 17; y++)
        ...
        public static bool SetGrayPalette(this Bitmap self)
        {
            if (self == null)
                throw new ArgumentNullException("self is null.");

            if (self.PixelFormat != PixelFormat.Format8bppIndexed)
                return false;
                ...
                for (int x = 0; x < w; x++)
                {
                    if ((fUp & (a * x + b > y)) | (!fUp & (a * x + b < y)))
                    {
                        ...
                        if (rct.Contains((float)xx, (float)yy))
                        {
                            ...
                            ImageUtility.intBicubic(dst, src, x, y, xx, yy);
                        }
                        else
                        {
                                        ...
                                        double radius)
        {
            ...
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                {
                    ...
                    if (rct.Contains(new PointF((float)xx, (float)yy)))
                        ImageUtility.intBilinear(dst, src, x, y, xx, yy);
                }
            ...
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                {
                    ...
                    switch (ro)
                    {
                        ...
                        case RankOp.Mid:
                            dst.R = ImageUtility.AdjustByte((rmax + rmin) / 2);
                            dst.G = ImageUtility.AdjustByte((gmax + gmin) / 2);
                            dst.B = ImageUtility.AdjustByte((bmax + bmin) / 2);
                            break;
                                                 ...
                                                 int radius, double angle)
        {
            ...
            angle = -angle * 30d;
            Bitmap tmp2 = ImageUtility.BitmapRotate(tmp, (float)angle, Color.Green);
            tmp.Dispose();
